package com.company.Mar.LinkedListPck;



// Java program to reverse alternate
// nodes of a linked list and append
// at the end

class LinkedListP {

    static Node head;

    static class Node {

        int data;
        Node next;

        Node(int item)
        {
            data = item;
            next = null;
        }
    }

    /* Function to reverse all even
       positioned node and append at the end
       odd is the head node of given linked list */
    void rearrange(Node odd)
    {

        // If linked list has less than 3 nodes,
        // no change is required
        if (odd == null || odd.next == null
                || odd.next.next == null) {
            return;
        }

        // even points to the beginning
        // of even list
        Node even = odd.next;

        // Remove the first even node
        odd.next = odd.next.next;

        // odd points to next node in odd list
        odd = odd.next;

        // Set terminator for even list
        even.next = null;

        // Traverse the  list
        while (odd != null && odd.next != null) {

            // Store the next node in odd list
            Node temp = odd.next.next;

            // Link the next even node at the
            // beginning of even list
            odd.next.next = even;
            even = odd.next;

            // Remove the even node from middle
            odd.next = temp;

            // Move odd to the next odd node
            if (temp != null) {
                odd = temp;
            }
        }

        // Append the even list at the end of odd list
        odd.next = even;
    }

    void rearrangeOrder(Node odd)
    {

        // If linked list has less than 3 nodes,
        // no change is required
        if (odd == null || odd.next == null
                || odd.next.next == null) {
            return;
        }

        // even points to the beginning
        // of even list
        Node even = odd.next;
        Node evenFirst=even;
        // Remove the first even node
        odd.next = odd.next.next;

        // odd points to next node in odd list
        odd = odd.next;

        // Set terminator for even list
        even.next = null;

        // Traverse the  list
        while (odd != null && odd.next != null) {

            // Store the next node in odd list
            Node temp = odd.next.next;

            // Link the next even node at the
            // beginning of even list
            even.next=odd.next;
            even = odd.next;
            even.next=null;
            // Remove the even node from middle
            odd.next = temp;

            // Move odd to the next odd node
            if (temp != null) {
                odd = temp;
            }
        }

        // Append the even list at the end of odd list
        odd.next = evenFirst;
    }
    /* Function to print nodes in a given linked list */
    void printList(Node node)
    {
        while (node != null) {
            System.out.print(node.data + " ");
            node = node.next;
        }
    }


}
public class ReverseLinkedEven {
    public  void solve()
    {
        LinkedListP list = new LinkedListP();
        list.head = new LinkedListP.Node(1);
        list.head.next = new LinkedListP.Node(2);
        list.head.next.next = new LinkedListP.Node(3);
        list.head.next.next.next = new LinkedListP.Node(4);
        list.head.next.next.next.next = new LinkedListP.Node(5);
        list.head.next.next.next.next.next = new LinkedListP.Node(6);
//        list.head.next.next.next.next.next.next = new LinkedListP.Node(7);

        System.out.println("Linked list before calling rearrange : ");
        list.printList(list.head);

        System.out.println("");
//        list.rearrange(list.head);
        list.rearrangeOrder(list.head);

        System.out.println("Linked list after calling rearrange : ");
        list.printList(list.head);
    }
}
