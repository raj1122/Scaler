...
class node implements Comparable<node>
{
    ...
    public node( int x, int y,int dist,boolean isType ) {
        this.x = x;
        this.y = y;
        this.dist = dist;
        this.isType=isType;
    ...
    @Override
    public int hashCode() {
        return Objects.hash(x, y, dist, isType);
    }
...
public class DamagedRoad2 {

    public int solve(int[] A, int[] B) {
        Set<node> al=new TreeSet<>();
        int dirx[]={0,1};
        int diry[]={1,0};
        int n=A.length;
        int m=B.length;
                ...
                for (int k = 0; k < 2; k++) {
                    ...
                    int tempy = j + diry[k];
                        ...
                        if(i==0 )
                        {
                            ...
                            node t=new node(tempx,tempy,A[i],true);
                            al.add(t);
                        }
                        else
                        {
                            al.add(new node(tempx,tempy,B[j],false));
                        }
...
//        Collections.sort(al);
        ...
        for (node d:al             ) {
            ...
            if(d.isType==false) {
                long t1=( (m%MOD) * (d.dist%MOD))%MOD;
                ans = ((ans%MOD) + (t1%MOD))%MOD;
//                n = Math.max(1,n-1);
                n--;
            }
            else
            {
                long t2=((n%MOD) * (d.dist%MOD))%MOD;
                ans = ((ans%MOD) + (t2%MOD) )%MOD;
//                m = Math.max(1,m-1);
                m--;
            }
        }
